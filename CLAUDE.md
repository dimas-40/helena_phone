...
## 🔴 세션 핸드오프 (2026-09-13) — 티스토리 로그인 문제, 다음 세션 최우선 확인

> 전 세션 컨텍스트 꽉 참. WSL Claude가 정리한 해법·인수인계 전문:
> `tistory-naver/HANDOFF_2026-09-13_LOGIN_SOLVED.md` 반드시 먼저 읽을 것.
> 요약: 캡차는 WSL이 삼성인터넷 CDP로 세션 빌려주는 방식으로 우회됨(폰 혼자는 불가).
> 정본 파이프라인은 `make_pair.sh --tistory` 하나뿐 — skin_inject.py/login_hil.py/login_phone.py는
> 전부 몰라서 만든 중복이니 쓰지 말 것. GitHub 토큰 노출 발견함, 폐기 필요.
