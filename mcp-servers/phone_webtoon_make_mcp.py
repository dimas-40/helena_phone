#!/usr/bin/env python3
"""
phone-webtoon-make MCP — 폰 웹툰 메이크(제작). 드로잉이 아니라 프로덕션.

본질 (Boss 2026-09-18 확정):
  - 드로잉(손으로 관찰·추상화해 그리는 재미) = Boss가 따로 한다. 그 자체가 목적·엔드스테이션.
  - 이 MCP = 그리기 뒤의 귀찮은 제작(업로드·생성·편집·조립·발행·영상화)을 자동화해 삼킨다.
  - Boss는 코드 한 줄 못 치고 STT로 말한다 → 에이전트(Claude)가 이 코드를 만든다.
  - 이미지 생성 = 삼성 온디바이스(GUI 자동화·무료). 그리기는 기계가, 제작은 이 MCP가.
