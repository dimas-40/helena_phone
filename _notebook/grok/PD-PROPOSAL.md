...
규칙: 얼굴은 `image_gen` 금지. 항상 `image_edit` + 위 참조.  
대사 숏(컷2)만 `reference_to_video` + 보이스. 나머지는 입 다문 채 `image_to_video`.
...
| S01 S04 S06 | `image_edit`(도면/공간 참조) 또는 `image_gen`(공간만, 사람 없음) | `image_to_video` 6s |
| S02 S05 S07 S08 | `image_edit` + REF_FACE | `image_to_video` 6s |
| S03 | `image_edit` + REF_FACE_ANGER | **`reference_to_video`** 6s + 보이스 |

`image_to_video` 프롬프트는 한두 문장, 현재형, 카메라 하나.  
