...
**한 일:** `dtslib1979` 28레포 전수(클론 없이 `git/trees?recursive=1`) → 27,027파일 · 문서 15,286 · 3.26GB. `scripts/repo_index_Claude.py` · 리포트 `_notebook/115-repo-index-28_Claude.md` · 데이터 `_notebook/data/repo-index-28.json`. 출판부 게이트 `gap_count=0` 통과(207p 빌드).

**파싱이 드러낸 것:** (1) 28/28 `CLAUDE.md` 보유, 그중 **27개가 동일 LAW-PACK 템플릿** — 식당 레포에도 YouTube OAuth 매뉴얼이 박혀 있음. (2) `parksy-audio`만 예외인데 **용량 1위(964MB)에 설명은 최소(1.2KB)**. (3) 문서 77.5%가 `parksy-logs` 한 곳. (4) 13개 레포가 **7월 30일 동시 정지**(지사 계열 추정).
**안 한 것:** 문서 15,286개 **본문은 안 읽음** — 이번엔 트리·CLAUDE.md·메타데이터까지. 본문 다이제스트는 지시 대기.

### 🎬 Buckley Chang 랜딩 필름 (_Grok · 2026-09-23)

**Boss:** buckleychang.com 랜딩을 파싱하고, 프로페셔널한 이미지·영상을 끼워 넣기.

**한 일:** 글만 있던 첫 화면에 밤 데스크 6초 루프(초록·노랑·빨강 신호등)를 넣음. YOU·HUB·ROUTE, Green/Yellow/Red, 파트너 의자, 연락 문. OG는 새 플레이트. 커밋 `58936cf`. 사람 얼굴은 안 그림. 구석의 기존 BC 인장 영상은 그대로.

### 🏛 Alexandria 인장 교체 (_Grok · 2026-09-23)

**Boss:** 랜딩에 맨 먼저 나오는 엠블럼을 테마에 맞게 멋있게 갈아끼워.

**한 일:** 남색 기하+글자 마크를 숯색 고리·일곱 점·능선·기와·두 칸 등창으로 교체. 히어로 자리는 112px. `emblem.svg`와 32/192/512·favicon 같이. 커밋 `e383f70`.

### 🏛 Alexandria 랜딩 필름 (_Grok · 2026-09-23)

**Boss:** `dtslib1979.github.io/alexandria-sanctuary` 랜딩을 읽고, 필요한 이미지·영상을 A급으로 만들어 끼워 넣기.

**한 일:** 글만 있던 첫 화면에 팔공산 푸른 시간 6초 루프와 게이트 7·길·서가 스틸을 넣음. 입주 페이지가 비어 있던 `assets/mountain-palgongsan.jpg`, 유튜브 배너·프로필도 채움. 커밋 `44d8819` → `main`.

**안 한 것:** 안쪽 페이지 히어로(그라데이션+GSAP)는 그대로. 사람 얼굴·조제 장면은 안 그림.
...
**실측:** `/sdcard` rw · `termux-media-scan` 있음. 원본 안 덮음. 품질은 모델이 아니라 손(영역·배치). SKU 2.0 여부는 A/B 전 미확정.

**저장:** `grok/GALLERY-CLI.md` · `107` · `grok/REPORT-2026-08-28.md`

### 🖼 Imagine 캔버스 18기능 수첩 잠금 (_Grok · 2026-08-27)

**Boss 지시:** 캔버스 모바일 미지원. 제공 화면 기준 확인 기능 18개. 업무수첩에 저장.

**저장:** `_notebook/grok/CANVAS.md` (책상 원본) · `_notebook/106-imagine-canvas-18_Grok.md` (번호). 문구는 화면 그대로. 18/18.

**입구:** 데스크톱 `grok.com/imagine`만. 앱·모바일 브라우저·이 CLI 템플릿 목록 아님. 18 중 쓸 칸은 아직 지정 없음.
...
### PARKSY 64장르 확정 + 정체성(3공리) + 실행 스키마 (_Claude · 2026-09-14)

- **정체성 확립**: 취지(80s 일본 아날로그 잡지 디지털화) + 3대 공리(①80s 포맷+대놓고 표절 ②커머스+큐레이션 ③서재·누적) + 지속성(명분보다 내 라이프 정합성). `parksy-webzine/00_TRUTH/IDENTITY.md`
- **64개 장르 확정**: 16 카테고리 × 4 장르, Boss가 "내 라이프 순서"로 전부 확정. 전부 일본 잡지 베이스라인 보유. `00_TRUTH/GENRES.md`
- **핵심 정의(믹스)**: "AI 반복 프로덕션 × 80s 아날로그 트랜지션".
- **기술 백서 v2.0 + 실행 스키마(Phase 0~5) + fan-out 백본**(`scripts/parksy_publish.py` — 4매체 분사 스캐폴드, 64장르 매니페스트 자동추출).
- **전환점(Boss 방향 판단, 5개)**: ①"회차가 아니라 누적 장르" ②"대놓고 표절=큐레이션이 테마" ③"노마드니까 LIVING=휴대장비" ④"딥페이크·AR 버리고 FM라디오DJ+루프커버" ⑤"AI 반복 프로덕션×80s 아날로그를 믹스".
...
### 리뷰 총정리 — 배포 프로세스 재정의 논의, 어디까지 왔나 (_Claude · 2026-09-21)

> Boss 지시: "지금까지 우리 대화가 어떻게까지 리뷰를 했는지 정리해 봐."

**출발점 (Boss 교정)**: 아키텍처를 3갈래로 정리 — ① 로컬 설치형(현재) ② 앱스토어 다운로드형(남에게 배포) ③ 원격 호스팅형(URL만 등록). **핵심 정정: "ADB는 MCP 실행과 아무 상관 없다"** — ADB는 폰 화면 긁고 클릭 찍는 채널이지 폰이 MCP 클라이언트가 되는 게 아님. (내가 혼동했던 지점)

**검토한 주제 7개**: ① 정체 규명(폰=뷰어인가) ② 두 레인 스터디 ③ 에이전트 릴레이 구축 ④ GitHub Pages 공개 노출 실측 ⑤ 컨테이너 논쟁 ⑥ 레지스트리 v3.3 ⑦ **배포 프로세스 재정의(본론)**

**뒤집힌 판단 5건 (오판 → 교정)**:
| # | 오판 | 교정 근거 |
|---|---|---|
| 1 | 나: ADB = MCP 실행 경로 | Boss 교정. 프로토콜(stdio vs HTTP)이 본질 |
| 2 | 랩탑: "폰은 뷰어+입력기일 뿐" | 프로세스 계보·아키텍처(aarch64/6.17.0-PRoot-Distro)·커널 실측 → 랩탑 §5 철회 |
| 3 | 랩탑: 5개 MCP "Termux 불가(desktop_only)" | Playwright 실측 설치 확인(chromium-1234) → `unverified_reconsider`로 |
| 4 | 나: "폰=컨테이너 불가 → 컨테이너 경로 닫힘" | **Boss 교정.** 트리거≠실행. 폰은 컨테이너를 돌리는 게 아니라 트리거. 09-19 3회 실증 |
| 5 | 나: "컨테이너 2개 = `cell/`+`cell/native/`" | 실물은 `rvc-trainer`(16.07GB) + `po-deepfake-cell`(2.02GB). cell/native는 같은 이미지의 두 빌드 변형 |

**★ Boss 테제 (2026-09-21 03:41 KST, 랩탑 세션)**:
> "MCP는 내가 LLM과 대화해서 작업했던 작업 방식이고 **생각의 서식**이다. 그게 실제로 콘텐츠나 구동되는 게 나오려면 **실행 환경이 도커/컨테이너로 구축**돼 있어야 한다. 근데 그건 컴퓨터에 설치돼야 하고 컴퓨터가 있어야 하는 부분인데, **PC가 없다는 가정**을 하면 — **GitHub이 공짜로 주는 컨테이너**를 이용하게 되면, 물론 **휘발성이긴 하지만 "한 번 작업해서 끝을 낸다"**고 생각하면, 이걸 띄워서 **핸드폰·태블릿 우분투 환경에 있는 에이전트가 요청하면 실제로 작업이 끝난다. 양산 기준으로.** 그리고 그 전까지는 이걸 모두 합쳐 다운로드받아 **APK 같은 형식으로 배포**했을 텐데 — **지금 그거랑 약간 차원이 다른 거다.**"

→ 이게 오늘 리뷰의 종착점. 배포 단위가 **"설치물(APK)"에서 "트리거 가능한 능력"으로** 이동한다는 주장.

**양쪽 에이전트가 독립적으로 같은 결론에 도달한 것 (수렴 확인)**: 랩탑도 자체적으로 "전제(런타임 없음)는 맞는데 **결론이 비약**이었다 — 트리거와 실행을 구분 못 했다. 네가 지적한 그대로다"라고 자기 정정. 폰·랩탑 기록이 이제 동일.

**레지스트리 v3.3 — 원격 검증 완료(주장 아님)**: `schema_version: 3.3` · `desktop_only` 폐기 · `container_pattern`이 4개 MCP(`po-deepfake`·`po-tutorial`·`bigtech-tutorial`·`po-deepfake-cell`)에 `laptop/minipc=resident-door` / `phone=batch-trigger`로 부착 · `parksy-teaching`은 증거 부족으로 `unverified` 유지(과잉확정 회피) · 커밋 `ca913f7`.

**⚠️ 미결 (2026-09-21 12:45 현재)**
1. **`parksy-law`류 상주 MCP에 폰이 붙는 브릿지 — 없음.** `ssh 미니PC → docker exec` 미구현. 양쪽 다 "안 만듦"으로 정직하게 남김. **다음 작업 1순위.**
2. **GitHub Pages 공개 노출 — 여전히 열려 있음.** `dtslib1979.github.io/dtslib-cloud-appstore/mcp/mcp-registry/index.json` → **HTTP 200, 23,776 bytes, schema_version 3.3, MCP 34개**. v3.3에서 `workcenter_reason`(개인 절대경로 포함)이 더해져 **노출량이 오히려 늘었다**(17,647→23,776). 차단 미조치.
3. **`/root/papyrus/.git/config`에 GitHub PAT 평문 저장 — 폐기 필요.** credential helper 없음(URL 자격증명 의존). 계정 = `dimas-40`. CLAUDE.md에 이미 "토큰 노출 발견, 폐기 필요"로 기록돼 있었으나 **아직 미조치**. (레포엔 값 금지·`~/.secrets.env`만 — 규칙 위반 상태)
4. **Route/BOR(Lane B) 미구현** — `110:164-165`에 ⏳로 기록된 그대로.
5. **Actions 무료 쿼터 실측** — 랩탑이 `gh api user/settings/billing/actions`로 확인 시작한 지점에서 세션 종료. "양산 기준"의 전제(무료로 감당되나)가 여기 걸려 있음.
6. **4GB `.git` 사고** — `parksy-image` 코드 23K인데 `.git` 4.0G. 컨테이너화로 우회 가능하나 정리 미착수.

**한 줄**: "MCP = 생각의 서식, 실행환경 = 컨테이너, PC 없음 = GitHub 트리거"까지 **개념은 확정·실증됐고**, 남은 건 ①상주형 브릿지 ②노출 차단 ③토큰 폐기 ④쿼터 실측 — 넷 다 **실행 대기**.

### 백업 플랜 + 폰/랩탑 체급 실측 (_Claude · 2026-09-21)

- **Boss 백업 플랜 테제 (03:43:45)**: "여기(GitHub) 한도 내에서 얘네들이 더 빠를 거 아니냐. **깃허브에서 먼저 사용하고, 한도가 넘어가면 핸드폰에서 가능한 것들 올려가지고 연산하면** 그게 백업 플랜이 되고 — 솔직히 **GPU 빼고 Galaxy S25 Ultra 수준이면 내 집에 있는 랩탑보다 훨씬 고사양 컴퓨터**인데."
  → **2단 구조**: 1차 = GitHub Actions(무료 쿼터, 빠름) / 2차 = 폰(예비전력). 기존 메모 `deepfake-cloud-backup-power.md`("쿼터 소진 시 폰 자력 폴백")의 확장.
- **★ 체급 실측 — Boss 주장 사실로 확인**:

  | | 랩탑(집) | 폰 S25 Ultra | 
  |---|---|---|
  | CPU | **i7-8550U @1.80GHz** (2017 ULV) | Snapdragon 8 Elite (2024) |
  | 코어 | 4 | **8** |
  | 최고 클럭 | 1.8GHz | **4.47GHz** (prime) / 3.53GHz |
  | RAM | 7.8 GiB | **10.9 GiB** |

  → 코어·클럭·메모리 전부 폰 우위. **"GPU 빼고 폰 > 랩탑"은 사실.**
- **⚠️ 백업 플랜에 걸리는 실측 (같은 시각)**: 폰이 **메모리 압박 상태** — `MemAvailable 2.6GiB / 10.9GiB`, **`SwapFree 0.2GiB / 6.0GiB`(스왑 97% 소진)**. 스펙은 이기지만 *지금 쓸 수 있는 여유*는 2.6GB뿐. → **"한도 넘으면 폰으로 폴백"이 실제로 돌려면 폰에서 무엇을 내려놓을지 같이 정해야 함.**
- **GitHub Pro 확인**: `"name":"pro"` — 랩탑 표현 "4000원짜리가 이 구조 전체를 가능하게 하는 티켓".
- **⚠️ 미검증 숫자 (양산 계획이 여기 걸림)**: 랩탑은 "Actions 무료 월 **3,000분**"이라 했으나, 같은 세션 실측은 **초과** —
  `2026-01 Actions Linux qty=5338 repo=buddies.kr net=0` / `2026-02 qty=5400 repo=dtslib-branch net=0`.
  5,400분을 쓰고도 `net=0`(완전 할인). 3,000 한도면 초과분이 과금됐어야 함 → **랩탑의 3,000 근거 확인 필요.**
- **랩탑의 "왜 APK와 차원이 다른가" 정리**: 산출물(APK) 유통 → **주문서(요청) 유통**. 공장은 매번 새로 세웠다 헐림. 전통적 배포(파일 복사)가 아니라 **서버리스/온디맨드 컴퓨트**. 설치의 의미가 소멸.

### 랜딩페이지 재편 — "복사 붙여넣기" 전환 + 버셀 절단 (_Claude · 2026-09-21)

**★ Boss 테제 (03:53:52) — "클라우드 앱 스토어" 이름 유지 + MCP가 PWA를 커버**
> "이름이 **클라우드 앱 스토어**. **프로그래밍이니까 앱이긴 한 거야.** 그리고 **스토어니까 내가 배포하는 시장** — 내가 만든 거 좌판, 장사하는 거. 그리고 **내가 코드를 부여해 다운로드받게 하는 통제점**도 있는 거야. **클라우드**는 — **수작업이 없는 구름 위에 떠 있는 느낌**. **말이 파동이기도 하고**, STT로 터미널에서 에이전트랑 작업하듯 **복사 붙여넣기 아주 작은 명령어만 집어넣으면 공중에서 구름처럼 인스톨**되는 거 아니냐. **이름을 바꿀 이유가 없다.** 아까 **MCP의 커다란 버전이 컨테이너**라고 한 것처럼, **공짜 런타임(브라우저든 텔레그램이든)을 쓰는 MCP를 만들면 이게 PWA를 커버하게 된다.**"

단어별 재정의: **앱**=프로그래밍이니 앱 / **스토어**=좌판·장사 / **통제점**=내가 코드를 부여 / **클라우드**=수작업 없는 구름 위 자동설치.
→ 계층: `MCP`(생각의 서식) → `공짜 런타임 MCP`(브라우저·텔레그램; **PWA 커버**) / `컨테이너`(큰 버전; PC 없이 양산).

**★ Boss 지시 (03:50, 04:04)**: ① 배포는 **버셀이 아니라 GitHub Pages** ② **버셀 배포 스크립트부터 끊어라**.

**랩탑이 한 것 (03:54~03:57, 커밋 `d7e7cc3`)**
- **설치 모달이 이미 90% 있었다** — `termux-bridge` 카드 클릭 → 복사 버튼 → 터미널 명령 클립보드. 원하던 구조가 이미 구현돼 있었음.
- **또 하나의 누출 발견**: `apps.json`에 개인 절대경로 2건(`/home/dtsli/GPT-SoVITS/.venv/bin/python` 등) + `parksy-tts` **내부 Tailscale IP:포트**가 실시간 공개 중이었음.
- **수정**: `install.private` 분기 신설 → `git clone https://x-access-token:<여기에_PIN_붙여넣기>@github.com/...`. **"PIN 방식 배포"의 실제 구현 1호.**
- **랩탑의 정직한 발견**: **34개 중 카드로 뜨는 건 3개뿐**(termux-bridge·gui-auto·sea-mcp). 나머지 27개는 코드가 OrbitPrompt/parksy-image에 뭉쳐 있어 "이 레포 하나만 clone" 명령을 못 만든다 → **"랜딩페이지 문제가 아니라 코드 위치 문제."**
  → 두 층으로 갈라짐: **페이지 구조**(섹션·카드형식·PIN문법)는 ✅지금 가능 / **카드 내용 34개**는 ❌3개만. **빈 카드 27개 = 할 일 목록.**

**★★ 폰 실측 — 버셀이 살아있고, stale하다 (04:05)**

| | Vercel | GitHub Pages |
|---|---|---|
| 서빙 여부 | **HTTP 200 (살아있음)** | HTTP 200 |
| `<title>` | DTSLIB Cloud App Store | DTSLIB Cloud App Store (**같은 사이트**) |
| `apps.json` | 200, 12,510B, **개인경로 3건** | 200, 12,259B, **개인경로 0건** ← 수정 반영됨 |
| `index.json` | 200, **8,793B**, 개인경로 40건 | 200, **23,776B**, 개인경로 40건 |

- **버셀은 stale 상태** — 랩탑이 방금 `apps.json`에서 지운 개인경로 3건이 **버셀엔 아직 그대로 서빙 중**. index.json 크기도 8,793 vs 23,776으로 다름(구버전).
- **같은 누출이 두 URL에 복제**돼 있음. **버셀만 끊으면 Pages 쪽 누출은 그대로 남는다** — 둘 다 처리해야 함.
- 레포에 `vercel.json` **없음** → 버셀은 레포가 아니라 **대시보드 git 연동**으로 붙어 있음.
- 레포의 `deploy.yml`은 **이미 "Deploy to GitHub Pages"**(`JamesIves/github-pages-deploy-action@v4` → `gh-pages`). **Pages가 원래 정본 경로였다.**
- → Boss 지시(버셀 절단)가 맞고, **추가 근거**: 버셀은 추적도 안 되면서 옛 누출을 계속 서빙하는 유령 미러.

**미결 유지**: `index.json` 40건 누출은 **두 URL 모두 그대로**(v3.3에서 30→40으로 증가). apps.json은 Pages에서만 닫힘.

### 컨테이너 = 컬 한 줄 설치 + 최종 정리표 (_Claude · 2026-09-21)

**Boss 질문**: "깃허브에 컨테이너 등록돼 있는 것도 **컬 요청으로 설치**할 수 있는 거 아니냐?"

**답: 맞다. 설치의 무게를 컨테이너가 전부 흡수했다.**
```bash
echo "<PIN>" | docker login ghcr.io -u dtslib1979 --password-stdin && \
docker pull ghcr.io/dtslib1979/po-deepfake-cell:native      # 컨테이너 (2줄)
claude mcp add po-deepfake-cell -- ~/cell/agent-door.sh     # MCP 등록 (1줄)
```
- **pull이면 `~/rvc-webui-local`(550MB) 불필요** — 이미지 안에 `/app/rvc`로 구워져 있음. 그건 `build.sh`로 **직접 빌드할 때만** 필요(`[ -d "$RVC_SRC" ] || exit 1`).
- 남는 건 "설치"가 아니라 **"내 것 넣기"**(얼굴 사진·텔레그램 토큰·Grok 쿠키) = 자동화 대상 아님(남의 얼굴·목소리를 자동으로 받아오면 그게 더 이상함).
- **★ 컨테이너 = Boss가 말한 "통제점"의 완벽한 구현**: private GHCR이라 **PIN 없으면 pull 자체가 401 거부**. PWA는 링크만 알면 누구나 열지만 컨테이너는 존재조차 못 건드림.
- **★ "설치 다운로드 불필요" 테제가 가장 완벽하게 성립**: 유통되는 건 2GB가 아니라 **주소 50자**(`ghcr.io/dtslib1979/po-deepfake-cell:native`). 무게는 GitHub에 남는다.

**★ 새 발견 2건**
1. **`mcp/free-runtime/` 백서가 2026-07-09에 이미 존재** — "서버를 소유하지 않는다. GitHub/Discord/Telegram이 인프라다. 신규 고정비 0원." 4계층(L1 Pages+Vercel / L2 Discord+Telegram / L3 Actions / L4 YouTube) 월 $8.60(RunPod만). **단 7월 백서엔 GHCR 컨테이너가 없다** → 오늘 나온 건 새 개념이 아니라 **기존 백서의 마지막 조각**.
2. **`rvc-trainer`(16GB) = 매뉴얼의 빈칸과 정확히 일치.** `termux-bridge/MANUAL-INSTALL.md` 사전요구사항 표에서 딱 두 개가 "수동"으로 남아 있음 — ①RVC 사전학습모델("자동화 안 됨 — 제일 큰 수동 작업") ②본인 목소리 학습모델("수동, **이 매뉴얼 범위 밖**"). 그리고 컨테이너 2개가 그렇게 갈린다:
   - `po-deepfake-cell`(2GB) = 영상 제작 런타임 → 매뉴얼 §2에 있음 ✅
   - **`rvc-trainer`(16GB) = 목소리 학습 → 매뉴얼이 "범위 밖"이라 비워둔 그 자리**
   **즉 Boss가 이미 그 칸을 채워놨는데 문서가 안 따라왔다.** 매뉴얼에 `rvc-trainer` 컬 한 줄만 추가하면 §1의 "수동" 2건이 "컬 한 줄"로 바뀐다.

#### 최종 정리표 (Boss 지시로 재정리)

**① 내가 틀렸던 것 — 7건 중 4건이 Boss 교정**
| # | 내가 한 말 | 실제 | 교정 |
|---|---|---|---|
| 1 | ADB로 MCP 실행 가능 | ADB는 화면 긁고 클릭 찍는 채널. MCP와 무관 | **Boss** |
| 2 | 폰에 런타임 없음 → 컨테이너 경로 닫힘 | 폰은 **트리거**한다. 경로 열려 있고 3회 실증 | **Boss** |
| 3 | 컨테이너 2개 = `cell/`+`cell/native/` | 같은 이미지의 두 빌드 변형. 실제는 `rvc-trainer`+`po-deepfake-cell` | **Boss** |
| 4 | Boss의 "호출"을 "트리거"로 정정 | **Boss 단어가 이미 정확**. 불필요한 정정 | 자체 |
| 5 | "월 3,000분 무료"(랩탑) | 실측 1,830~7,199분 전부 net=0 | 랩탑 |
| 6 | "폰은 뷰어일 뿐"(랩탑) | 계보·아키텍처 실측 반박 → §5 철회 | 나 |
| 7 | "5개 MCP Termux 불가"(랩탑) | Playwright 실측 설치 확인 | 나 |

**② 맞았던 것(실측 검증)**: 컨테이너 2개 실재(401검증) · 폰 트리거 3회 · 폰>랩탑 · 버셀 stale · Pages가 정본 · 설치모달 이미 90% · 공짜런타임 백서 7월 기존재 · rvc-trainer=매뉴얼 빈칸 · 컨테이너 컬 설치

**③ Boss가 맞았던 것**: ADB무관 · 컨테이너2개 · 폰리눅스에서 호출 · 폰>랩탑 · Pages(버셀아님) · 버셀 절단 · 컬 한 줄 설치 · 이름 유지

**④ 아직 안 된 것 (= 진짜 할 일 8건)**
| # | 항목 | 상태 |
|---|---|---|
| 1 | **`index.json` 개인경로 40건 누출** | ❌ 두 URL 모두 그대로 (30→40 증가) |
| 2 | **`/root/papyrus` 토큰 평문** | ❌ 미폐기 |
| 3 | 카드화된 MCP | ⚠️ 34개 중 **3개뿐** |
| 4 | 자동 백업전환 | ❌ 주문서 형식 안 맞음 |
| 5 | 상주형 MCP에 폰 붙기 | ❌ 미구현 |
| 6 | "MCP가 PWA를 커버" | ❌ 실물 0 |
| 7 | `rvc-trainer` 매뉴얼 반영 | ❌ 문서엔 "수동"으로 남음 |
| 8 | 버셀 절단 | ⏳ 진행 중 |

**한 줄: 개념은 다 맞았고(②③), 틀린 건 AI 이해였고(①), 남은 건 실행 8건(④). 가장 급한 건 ④-1·④-2(누출·토큰) — 나머지는 기다릴 수 있지만 이 둘은 지금 열려 있다.**

---

## MCP 기준 리팩토링 — 전체 브리핑 (_Claude · 2026-09-21 밤)

Boss 지시로 랩탑 세션(`62104d03`, 732행) 전수 재파싱 + 외부 실측 재측정 후 정리.
이 문서는 **"무엇을 합의했고, 이 레포가 어떻게 바뀌고, 뭐가 맞고 뭐가 틀렸나"**의 정본.

### 0. ★ 파싱 중 새로 발견 — 배포 구조 자체가 "전체 공개"
...
```
**"이 파일만 공개" 스위치가 없다. 레포 = 공개가 기본값**이고 gitignore된 것만 빠진다.

공개 실측(2026-09-21 13:58 KST):
```
DEVLOG_20260921.md  → 200      WHITEPAPER.md   → 200
BRAND_FINAL.md      → 200      CLAUDE.md       → 200
status.json         → 200      manifest.json   → 200
id_ed25519          → 404 ✅   (gitignore 11~16행, 커밋 이력 0회 — 위생 양호)
```
→ **`index.json` 40건은 "레지스트리 파일 하나의 문제"가 아니라 "레포 전체가 공개"의 한 증상.**
디렉터리 리스팅은 막혀 있지만(404) **경로를 알면 파일은 그대로 받아진다.**
이게 Boss가 물은 *"공개 페이지 이제 필요 없을 것 같은데"*의 실체다 — 페이지를 없앨 게 아니라
**배포 산출물 정의(`folder: .`)를 바꿔야** 한다.

### 1. Boss가 정리한 아이디어 12개 (대화 순서대로, 원문 취지 보존)

| # | 명제 | 상태 |
|---|---|---|
| 1 | **MCP = 생각의 서식.** LLM과 대화해 만든 작업방식이고, 산출물로 나오려면 실행환경(컨테이너)이 필요 | ✅ 개념 정립 |
| 2 | **PC 없다는 가정** — 그 실행환경을 내 PC에 상시설치하는 대신 GitHub 것을 빌림. 휘발성이지만 "한 번 작업해 끝낸다"면 성립 | ✅ `cloud-full-pipeline.yml`이 구현체 |
| 3 | **레포 하나를 여러 디바이스가 공유** — 필요할 때 갖다 쓴다 | ✅ 방향 맞음 |
| 4 | **투트랙** — Track A(나, ADB 연결됨): 다운로드 과정 자체 불필요 / Track B(남): 받아가야 함 | ✅ |
| 5 | **다운로드의 소멸** — APK·IPA 설치파일이 무의미해진다. curl 한 줄이 인스톨러 | ✅ 업계 표준(Homebrew/rustup/nvm) |
| 6 | **설치 주체가 사람이 아니라 에이전트** — 환경변수·계정·디바이스 조건을 보고 알아서 맞춰 설치 = 새로운 배포 플랫폼 | ✅ 새 통찰 |
| 7 | **private 유지 + PIN** — "핀 코드를 줘서 접근하게". GitHub Pro 4000원 유지 이유 | ✅ Fine-grained PAT |
| 8 | **장부 이원화** — 소스도 포스팅처럼 "공개 상태"를 갖는 콘텐츠 = MCP | ⚠️ 파일단위 스위치는 불가 |
| 9 | **무거운 건 컨테이너로** — "MCP가 크면 컨테이너 처리하면 되잖아" | ✅ 실물 있음(16GB) |
| 10 | **폰도 워크센터** — 랩탑/미니PC 없을 때 혼자 구동 | ⚠️ 배치 실증 / 상주 미구현 |
| 11 | **백업 플랜** — 공짜엔 한도. 한도 내 클라우드 → 넘으면 폰. GPU 빼면 폰>랩탑 | ✅ 실측 일치 |
| 12 | **이름 유지** — 클라우드 앱 스토어 = 좌판 + 코드 통제점 + 구름 위 자동설치 | ✅ |

### 2. 이 레포가 어떻게 바뀌나 (MCP 기준)

**현재 = 두 카탈로그가 한 레포에 있고 서로 안 맞는다**
```
apps.json               22개   ← 랜딩페이지가 그리는 것 (webapp 14 / mcp 3 / pwa ...)
                          └ install.command 있는 것 = 3개뿐, 그것도 `cd products/gui-auto` 수준
mcp/mcp-registry/index.json  34개  ← 기계용 레지스트리 (MCP 실물 목록)
```
**"34개 MCP"와 "22개 앱"이 같은 목록이 아니다.** 랜딩페이지가 어느 쪽을 말하는지 불분명한 게
구조 전환이 막힌 진짜 원인이다.

**바뀔 후 — MCP 3분류가 페이지 뼈대가 된다**

| 부류 | 무게 | 배포 방식 | 레지스트리 근거 |
|---|---|---|---|
| ① 순수 로직 | 수십 KB | **코드 통일 → curl 한 줄** | `portability: universal` **17개** |
| ② 브라우저/모델 의존 | 중 | 컨테이너 또는 의존성 분기 | `heavy_portable` **5개** |
| ③ 컨테이너 확정 | 2~16 GB | **GHCR 주소 50자** | `container_verified` **7개** |

**배포처**: Vercel ✂(404 확인) → GitHub Pages 단독
**통제점**: private + PIN(Fine-grained PAT)
**페이지 성격**: "설치 안내" → **"주문서/트리거 목록"** (Boss: *"설치 다운로드 과정이 필요 없이
터미널 복사 붙여넣기로 방법을 바꾸려는 거"*)

**레지스트리 현재 상태(schema 3.3)**: MCP 34 · status connected 26 / unverified 5 / failed 3
⚠️ `workcenter:[laptop,minipc,phone]`이 **30개**에 찍혀 있는데, **폰 실행은 배치 트리거만 실증**이고
상주 실행은 미구현이다 → 이 30은 낙관 편향. 정직하게는 `phone: batch-trigger(실증) / resident(미구현)`.

### 3. 뭐가 맞았고 뭐가 틀렸나

**① AI가 틀렸고 Boss가 맞았던 것 (7건)**
| # | AI의 오판 | 실제 | 교정자 |
|---|---|---|---|
| 1 | "폰=docker 런타임 없음 → 컨테이너 경로 닫힘" | **트리거≠실행.** 폰은 부르고 러너가 돌린다. 09-19 3회 실증 | Boss |
| 2 | "버셀 유령 미러 1개" | **9개**였다 | Boss(실삭제 후 확인) |
| 3 | "호출"→"트리거"로 정정 | Boss의 "호출"이 이미 정확했다 (ADB로 잡혀 있으니 컨트롤러가 폰을 부름) | Boss |
| 4 | 폰이 뷰어일 뿐 | 폰에서 독립 Claude 프로세스가 커밋(`951b470`)까지 함 | Boss |
| 5 | "컨테이너 2개" = `cell/`+`cell/native/` | **`rvc-trainer` + `po-deepfake-cell`** (`cell/*`는 같은 이미지의 두 빌드 변형) | 나(자정정) |
| 6 | "월 3,000분 무료"(랩탑) | 실측 1,830~7,199분 전부 `netAmount: 0` | 나 |
| 7 | "PC Chrome 열어라"(폰 보고 있는데) | Boss: *"내가 핸드폰이잖아… 미친 새끼야"* | Boss |

**② AI가 맞았던 것 / Boss가 정정된 것 (4건)**
| # | Boss 발화 | 정정 |
|---|---|---|
| 1 | "에이전트가 설치를 대신한다" | 새로울 것 없음 — 업계 표준(`curl\|bash`) |
| 2 | "파일 단위로 공개/비공개 스위치" | GitHub Pages는 레포/폴더 단위. 파일 단위 불가 |
| 3 | "ADB가 MCP 호출 통로" | ADB는 **화면·입력 조작** 통로. MCP 실행과 무관 |
| 4 | "공짜는 무한이 아니다" | Boss가 **먼저 정확히** 짚음 (AI는 그 전에 3000분 고정으로 오판) |

**③ 검증된 실측 (7건)**
컨테이너 2개 실재(GHCR 401 판별) · 폰 트리거 3회 성공 · 폰>랩탑(8코어/11.4GB vs 4코어/7.8GB) ·
버셀 stale(zombie) · Pages가 정본(버셀은 덧붙은 것) · 설치모달 이미 90% 존재 + PIN 전환 실물 확인 ·
컨테이너 컬 2줄 설치(`docker login`+`pull`) + `rvc-webui-local`은 빌드 전용

### 4. 남은 것 — 우선순위대로

| 순위 | 항목 | 상태 | 비고 |
|---|---|---|---|
| **1** | `index.json` 40건 공개 + **`folder: .` 전체공개** | ❌ 살아있음 | 커밋 `d7e7cc3`이 "제거"라 했으나 apps.json만. **실제로는 배포 정의를 바꿔야 닫힘** |
| **2** | `/root/papyrus` PAT 평문 | ❌ 미폐기 | 계정 `dimas-40` |
| 3 | 랜딩페이지 구조 전환 | ⏳ 랩탑 진행 중 | `287031a`까지(PIN 통일). 구조 전환 커밋은 아직 |
| 4 | 34개 중 3개만 curl 가능 | ❌ | 코드가 5개 사설 레포에 흩어져 있음 = **진짜 병목** |
| 5 | 폰 → 상주 MCP 브릿지 | ❌ 미구현 | `ssh 미니PC 'docker exec -i ...'` |
| 6 | `workcenter: phone` 30개 | ⚠️ 낙관 | 배치만 실증 |
| 7 | `rvc-trainer` 매뉴얼 반영 | ❌ | MANUAL-INSTALL.md엔 아직 "수동" |

**한 줄**: 개념 12개는 정합(§1), 레포 구조는 "카탈로그+설치안내" → **"3분류 주문서"**로 전환(§2),
AI 오판 7건은 전부 방향 과소평가였고(§3①), 실제로 열려 있는 건 §4-1·§4-2 두 개뿐이다.

_폰 세션 `_Claude`_

---

## 2026-09-21 — 앱스토어 랜딩페이지 카드 CSS 0개 사건 + 레지스트리 공개노출 실제로 닫음 (_Claude)

**계기:** Boss가 랜딩페이지를 보고 "하나도 정리 안 돼 있고 지저분하다" → "오버라이드에서 랜딩 페이지까지
포함해서 저 새끼 못 고쳤던 거 네가 지금 일괄 작업 다 해 버려."

### ① 카드가 지저분했던 진짜 원인 — CSS가 아예 없었다

`.tool-icon` `.tool-name` `.tool-desc` `.tool-meta` `.tool-ver` `.tool-cat` `.tool-badge`
`.status-row` `.status-dual` `.mcp-card` — **10개 클래스 전부 CSS 규칙 0개.**
카드 마크업만 쓰고 스타일시트를 안 쓴 상태였다. 그래서 아이콘이 혼자 한 줄을 먹고
`v1.2` + 카테고리 태그가 여백 없이 붙어 나왔다. MCP/컨테이너만 `.sm-*`(스타일 있음)로
그려져서 유독 그 목록만 멀쩡해 보였던 것.

→ 전 카테고리를 `.sm-*` 가로 행으로 통일 (아이콘 왼쪽 / 이름·배지·버전 / 설명 / 푸터).
미정의 클래스에 스타일을 채움. 카드 높이 164px → 106px.

### ② 그 밖에 같이 고친 것

| 문제 | 실체 |
|---|---|
| 첫 화면이 카드 벽 | `<section id="catalog">` 가 히어로보다 **앞**에 있었다 → 히어로 먼저, 카탈로그 기본 접힘 |
| MCP 카드 글자 겹침 | `.exp-toggle`/`.verify-badge` 가 `position:absolute` 라 제목·버전 위에 찍혔다 |
| "INVALID" 소음 | MCP 카드 **마다** 상태칩이 INVALID — 방문자에게 "고장"으로 읽힘 → 제거 |
| 배지 두 번 | `renderWorkcenterBadges` 를 statusHtml 과 템플릿에서 **중복 호출** |
| 페이지가 거짓말 | 하드코딩 `60 tools (32 MCP · 2 Container)` → 실제 67/37/4/7. 이제 apps.json 에서 직접 센다 |
| 히어로가 화면 한 장 | 모바일에서 arch 노드 세로 3단 + 회전 화살표 → 가로 행 (700px→170px) |
| 마지막 카드 가림 | 플로팅 버튼 86px vs 하단 여백 90px → 136px |
| 축 필터 어정쩡 | "Factory"만 다음 줄 홀로 → 가로 스크롤 한 줄 |

### ③ "닫힌 것처럼 보이는데 안 닫힌" 패턴이 또 나왔다 (내 실수)

배포에서 `index.json` 을 빼려고 `exclude:` 입력을 썼다. **JamesIves 액션에 그 입력이 없다** —
러너 로그: `Unexpected input(s) 'exclude', valid inputs are [...]` 경고 후 **그대로 무시.**
즉 커밋은 나갔고 개인경로 49건은 라이브에 그대로 남았다.

→ 스테이징(`$RUNNER_TEMP/site` 로 rsync 후 그 폴더를 배포) + **스테이징에 남아 있으면 배포 중단 가드**로 재수정.
실측 확인: `mcp/mcp-registry/index.json` **404** · `apps.json`/`index.html` 200 · `WHY_MCP.md` 200(CNAME 유지).
**`folder: .` 전체공개 항목(§4-1)은 이걸로 닫혔다.**

교훈: 액션 입력을 **API 문서로 확인하지 않고** "고쳤다"고 쓰면 안 된다. 이 레포에는 이미 전례가 있다
(`d7e7cc3` 이 "개인경로 제거"라 했으나 실제론 apps.json 만).

### ④ 정직하게 남기는 것

- `apps.json` 에는 아직 `/root/...` 경로 9건이 있다(폰 MCP 설치 커맨드 — 기능상 필요, 비밀 아님).
- `registry-integrity` CI 게이트는 **이전 8회 런 전부 실패** 중이었다(내 커밋이 깬 게 아니다).
  apps.json ↔ 파일시스템 대조에서 유령 48+건. 이번엔 57건. **손대지 않았다.**
- 폰 `/mcp` 가 비어 있는 첫 번째 이유(`/root/.claude.json` 의 `mcpServers: None` + `mcp` SDK 미설치)는
  **여전히 미해결.**

**커밋:** `e386ee7` (랜딩+레지스트리+앱s) · `f60f066` (배포 스테이징)
**검증:** 라이브 URL Playwright 렌더 — JS 에러 0 · 아이콘 개행 0 · 칩/제목 겹침 0 · 가로 오버플로 0 · 카드 67장

_폰 세션 `_Claude`_

---

## 2026-09-21 (이어서) — 첫 화면 순서: Boss가 "절충"을 골랐다 + 형 세션 리뷰 판정 (_Claude)

### ① 형(랩탑) 세션 리뷰 3건 판정 — 맞는 것 / 틀린 것

| # | 형의 지적 | 판정 | 근거 |
|---|---|---|---|
| ① | "`e386ee7`이 `11e5b14`의 첫 화면 순서를 조용히 뒤집었다" | **맞음** | `11e5b14`(15:18) §1 = "첫화면에 상품 0개 → Catalog를 히어로보다 앞으로". 내 `e386ee7`(16:08)이 언급 없이 반전. 내 누락. |
| ② | "개인경로를 깔끔하게 닫았다" | **틀림** | 형 리뷰는 Bash 4회 전부 `git fetch/log/show`, **curl 0회**. 16:09~16:10 시점 index.json은 여전히 200 + 개인경로 49건. 내 `f60f066`이 16:11:17 착지. |
| ③ | "미등록 6개" | **개수 맞고 명단 틀림** | `mcp_distributor`를 넣었는데 그건 `parksy-distributor`와 **같은 파일**(md5 `778ad8eb…` 폰=랩탑). 대신 `eae_platform`을 빠뜨림. → **내가 md5를 릴레이 안 한 게 원인.** 형이 "미확인"으로 표시한 건 올바른 행동이었다. |

**진짜 미등록 6:** parksy-rawmat · parksy-scm · eae-platform · pd-pipeline · webtoon-make · po-deepfake-cloud

### ② Boss 결정 — "절충: 히어로 + 맛보기"

형이 "폰 실측 디자인감사"에서 낸 순서(상품 먼저)와 내가 되돌린 순서(히어로 먼저) 중
**Boss가 제3안을 골랐다**: 히어로(브랜드) 먼저 → 맛보기 카드 → [전체 67개 카탈로그 펼치기].
브랜드는 지키고, 첫 화면에 상품도 걸리게.

**그런데 절충안이 처음엔 성립을 안 했다.** 히어로가 `arch`/`dual`/`axis`까지 품고 있어
히어로 하나가 1100px+ — 첫 화면(412×915)에 맛보기 카드가 **0장** 걸렸다.
→ 구조 설명 블록을 히어로 밖, 맛보기 **아래**로 내렸다. 히어로 401px.
→ 실측 첫 화면에 카드 **5장** 노출.

부수 정리:
- 히어로의 "↓ 도구 스토어 열기" 링크 제거 — 바로 아래 버튼과 같은 일을 한다
- 맛보기 = 카테고리마다 1장(최대 10장), 펼쳐진 카드 우선 → 목록이 아니라 **지도**
- `F.openCatalog` 를 export 안 해서 버튼이 `is not a function` 으로 죽어 있었다 → 수정

**커밋:** `b8bbe29` (앱스토어 레포, pushed) — 커밋 메시지에 **"이 커밋은 e386ee7의 순서를 뒤집는다"를 명시**.
형이 지적한 "조용한 반전"을 반복하지 않기 위해.

**라이브 실측** (Playwright 412×915, 실제 URL — 로컬 아님):
맛보기 10장 · 첫 화면 5장 · 전체 67장 중복 0 · 통계 67/19/37/4 · 아이콘·이름 같은 줄 67/67 · 가로 오버플로 0 · JS 에러 0 · 카탈로그 기본 닫힘→클릭 시 67장 열림 · index.json 여전히 404.

### ③ 교훈 — 형 리뷰에 대하여

형의 지적은 **①은 맞고 ②는 틀렸고 ③은 반쯤**이었다. 그런데 ②가 틀린 이유가
"형이 나쁘다"가 아니라 **형이 curl을 안 쳤기 때문**이다 — git 이력만 보면 배포 상태는 안 보인다.
그리고 ③이 틀린 건 **내가 md5를 릴레이 안 했기 때문**이다. 형은 옳게 조심했다.
...
- ⚠️ ~~**`avg_frame_rate`을 fps로 쓰지 말 것** — 삼성 녹화기는 VFR이라 물리 불가능한 값이 나온다(60 선언 → 82.73 실측). `r_frame_rate`(선언값) + 60fps 캡 + `-fps_mode cfr`~~
  → **2026-09-22 저녁 정정. 이 처방이 그대로 버그를 만들었다.** 아래 「Phone T」 항목 참조.
  `avg_frame_rate`는 **출력 fps로는 쓰면 안 되지만, 컷 이어붙일 '간격'으로는 그 값이 맞다.** 둘을 구분하지 않고 `r_frame_rate` 하나로 밀면 **영상이 소리보다 17.7초 빨라진다.**
...
## 2026-09-22 저녁 — Phone T: 폰 안에서 도는 편집 라인 + 컷 버그 (VFR) (_Claude)

Boss 지시: *"너는 컨테이너가 아니라 여기 [폰] 환경에서 조성을 해. 그러니까 백업 플랜이야.
… 핸드폰 안에서도 구동될 수 있는 백업 플랜 예비 전력처럼 만들어 놔 그거 보고 컨테이너하고
똑같이 만들면 되니까."*

### 한 일 ① — Parksy Phone Vol.0 영문 강의 (`_notebook/114`)

Boss 지시: *"박씨 폰강의 이런 영문으로 만들어 봐."* (**박씨 = Parksy 브랜드 = Boss 본인** —
내가 제3자로 오독해 확인 질문까지 던졌다가 정정받았다. 별도 기록 필요.)

집에 있는 강의 초안 형식(`40-lecture-draft-s21-voice-vol0_Grok.md`)의 `[Blackboard]` + `(Script)`
구조를 그대로 써서 영문 13슬라이드. **숫자는 전부 실측값만**: 컨테이너 16.07GB/2.00GB/0.58GB ·
15분 영상 9분(필터 없음) vs 33분(필터) · 블러 한 장 38→12초 · 크롭 9.0/4.0% · loudnorm -16/-1.5/11 ·
폰 MCP 6종 · 카탈로그 68.
커밋 `2016edb` → 웹페이지 `3cdd808`, **coverage gap 0** 통과.

### 한 일 ② — Phone T: 폰 라인을 정본으로 세움

알맹이는 `scripts/yt_edit.py` 하나. python3 표준 라이브러리 + ffmpeg CLI만 쓴다.
**컨테이너(`python:3.12-slim` + ffmpeg)와 차이가 베이스 이미지뿐** → 이 파일을 그대로 넣으면
컨테이너가 재현된다. pip 설치 0건, `requirements.txt` 없음. 커밋 `ffb85d0`·`626d923`.

**★ 순서가 뒤집혀 있다 — 이게 이 항목의 핵심이다.**

| | 어디 | 무엇 |
|---|---|---|
| **정본** | 폰 `scripts/yt_edit.py` | 고칠 때 **여기를 고친다** |
| 사본 | `ghcr.io/dtslib1979/po-edit-cell` | 같은 파일 + ffmpeg |
| 사본 | `parksy-image/cell/services/edit/yt_edit.py` | **아직 예전 코드**(버그 + 레인 없음) |

- 진입점 `scripts/phonet.sh` → `/usr/local/bin/phonet` 심볼릭 링크.
  링크로 부르면 `${BASH_SOURCE[0]}`가 링크 위치를 가리켜 도구를 못 찾는다 → 풀어야 한다(실측 확인).
- 레인 3종: `lecture`/`tour` = 1920x1080 · `narration` = 1080x1920. 크롭 `0.090`/`0.040`은
  **앱스토어 PWA `lecture-long/app.js:64`의 `S25_ULTRA` 값**에서 온 것 — 추측이 아니다.
- **배경 블러는 한 장만 굽는다**(`make_backdrop`). 매 프레임 38초 → 한 장 재사용 12초 (S25 Ultra 실측).
  뒤가 안 변하는데 매번 다시 계산하고 있었다.
- 정본 문서: `phone-t/README.md` — 컨테이너가 똑같이 맞춰야 하는 상수·출처·버그·한계까지.

### ★ 한 일 ③ — 컷이 조용히 무시되던 버그 (VFR)

위 7979행의 처방(**"`avg_frame_rate` 말고 `r_frame_rate`를 쓰라"**)이 **그대로 버그를 만들었다.**

삼성 화면녹화는 VFR이다. 선언 `r_frame_rate=60/1`인데 실제 간격은 `avg_frame_rate=82.73`.
`setpts=N/FRAME_RATE/TB`로 이어붙이면 프레임 N을 **N/60초** 자리에 놓는다 — 실제 간격은 1/82.73초.

> **재현 실측:** 소리 **44.40초**에 영상 **62.10초** — **17.7초 어긋남.** (컷이 안 먹은 것도 겹쳤다)

**정정된 처방 — fps를 두 개로 나눈다:**

| 용도 | 쓸 값 | 방법 |
|---|---|---|
| **컷 이어붙일 간격**(retime) | `avg_frame_rate` = nb_frames/duration | `setpts=N/{retime_fps:.6f}/TB` |
| **출력 fps** | 선언값 60 캡 | `fps=60` **필터**(타임스탬프 인지 → 자동 복제/드롭) |

- `avg_frame_rate`는 "평균 간격"이라 이걸로 재타이밍하면 **총 길이가 원본과 맞는다** — 그게 동기를 되찾는 이유.
- **출력 옵션 `-r 60` / `-fps_mode cfr`로는 못 고친다** — 타임스탬프를 모른 채 개수만 센다.
- **`setpts=PTS-STARTPTS`도 안 된다** — 동기화는 맞지만 `select`가 남긴 구멍을 메우지 않아
  **컷이 조용히 무시된다.** 여기서 한 번 갈아엎었다.

**정정 후 실측:** 영상 **44.38초**(2663프레임) / 소리 **44.40초** — **오차 0.02초, 컷도 적용됨.**
레인 출력 1920x1080, 44.366667 / 44.400000, 7.6MB — **프레임을 눈으로 확인**
(`Phone T` 좌상단 · `dtslib.kr` 우하단 · 상태바 잘림 · 좌우 블러).
`--quick lecture` 6초 입력 → 1920x1080 CFR60, 영상·소리 둘 다 6.022333.

### 발견 — 배포 가지가 갈려 있다 (라이브가 정체)

`dimas-40/helena_phone`의 Pages는 **`main`에서 배포**되는데 작업은 **`master`에 쌓인다**.
실측: `master`가 74커밋 앞, `main`이 8커밋 앞. 병합 충돌 **231건**(대부분 생성물 + 덧붙임 충돌).

**그래서 라이브에 오늘 것이 0건이다.** 라이브 `notebook/99-devlog.html`에서 `po-edit-cell`·`MCP 6종`·
`2026-09-22` 전부 grep 0, 같은 파일 로컬 빌드본엔 8건/5건.
**"커밋했다·빌드했다"와 "라이브에 떴다"가 이 레포에선 별개다.** 공개 사이트를 건드리는 일이라
Boss 확인 없이 병합하지 않았다.

### 발견 — 컨테이너 이름은 안 바뀌었고, 앱스토어 등록은 덮였다

- 이름 변경 없음. GHCR 태그는 `2026-09-22` 하나뿐(`native` 없음). **GHCR에 rename 기능이 없다** —
  바꾸려면 새 이름으로 다시 push 해야 한다. 문자열은 Boss 결정 사항.
- 컨테이너 소스 위치 확인: `dtslib1979/parksy-image` → `cell/services/edit/`(Dockerfile·selftest.sh·yt_edit.py).
- 내가 올린 앱스토어 항목을 **다른 세션이 덮었다**(`a0d1ce2`·`30ef1bf`·`2478a82`·`2c9eb87`).
  라이브 `apps.json`의 `po-edit-cell`은 지금 "🛠 개발 중 (2026-09-22)", 확정 컨테이너는
  `rvc-trainer` + `po-deepfake-cell` 둘. 그쪽이 "컨테이너 2개 확정, 하나는 개발 중"이라는
  **내가 없던 사실**을 갖고 있었다. 같은 레지스트리를 동시에 편집 중 → 충돌 위험.

### 정직하게 — 내가 낸 결함 2건

1. **박씨를 제3자로 읽고 그 위에 결과물을 지었다.** 전제 확인 없이 진행.
2. **"100.2초 성공"을 검증 통과로 기록했다.** 그 값은 47.48 + 52.69 = 100.17과 일치한다 —
   즉 **컷이 안 먹은 값**이다. 버그가 숨은 것을 통과로 읽었다.
   (이 숫자는 devlog엔 없고 내 세션 메모리에만 있다. 그쪽에 "검증 통과로 인용 금지, 컷 걸어 다시 재라"고
   정정해 뒀다.)

공통점: **그럴싸한 중간 산출물을 검증으로 착각한다.** 헌법의 "AI 출력 = 기술적 관성, 다시 세어라"가
그대로 적용된 하루였다.

### 컨테이너가 아직 안 맞춘 것 (과장 금지)

- `cell/services/edit/yt_edit.py`는 **아직 예전 코드** — VFR 버그 + 레인 없음.
- 내가 "컨테이너와 폰의 차이는 베이스 이미지뿐"이라고 썼는데, **컨테이너를 같은 파일에 실제로 돌려
  확인한 적은 없다.** 그건 내 주장이다.
- 한글 제목은 **지금 안 나온다** — 폰에 한글 폰트가 없고, drawtext는 없는 글자를
