...
╔══════════════════════════════════════════════════════════════════════════╗
║ ⛔ 폐기됨 (2026-09-23) — **쓰지 마라.** 정본은 `axis_arm.tv()` 다.        ║
╚══════════════════════════════════════════════════════════════════════════╝

이 스크립트는 **삼성 비디오 플레이어를 freeform 창으로** 띄운다. 실측으로 두 가지가
안 돼서 버렸다:

  ① 플레이어가 **재생 컨트롤(▶ · 탐색바 · 0:10)** 을 같이 띄운다 → 녹화에 그대로
     찍힌다. 창 위에 **파란 손잡이 막대**까지 붙는다. 이게 Boss 가 "지저분하다"고
     한 그 화면이다.
  ② **창이 1분을 못 버틴다.** 14:21 에 띄운 창이 14:22 에 사라졌다(스크린샷으로 확인).
     Boss 가 말한 "잠깐 떴다가 사라지는 거"의 정체.

정본은 Boss 가 2026-09-22 에 직접 만든 `TvOverlayService`(Axis v11.4.0 안의 네이티브
VideoView + 포그라운드 서비스)다 — `python3 scripts/axis_arm.py --tv`.
액자가 영상에 구워져 있어 컨트롤·장식이 **구조적으로 생길 수 없고**, 포그라운드
서비스라 죽지 않는다.

지우지 않고 남기는 이유: 여기 적힌 freeform 실측(창 배치·회전 무시·MediaStore 등록)은
**다른 작업에서 다시 쓸 수 있는 지식**이다. 특히 `bounds_for()` 의 좌표 계산과
`cmd window set-ignore-orientation-request` 발견은 삼성 앱을 구석에 앉히는 유일한 방법이다.
